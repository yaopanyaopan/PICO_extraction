...
def attention(inputs, attention_size, hidden_units,return_alphas,regularizer,name):
    ...
    with tf.variable_scope("attention"+name,reuse=tf.AUTO_REUSE):
        ...
        # 注意力机制
        # W_omega = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(),regularizer=0.0001,shape=[hidden_size , attention_size],dtype=tf.float32,name=name+"w")
        W_omega = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(), shape=[hidden_size,attention_size],regularizer=regularizer,
                                  dtype=tf.float32, name=name + "w")
