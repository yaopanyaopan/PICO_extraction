...
import numpy as np
import math
from Model_acl import Myattention
...
class BiLSTM_CRF(object):
    ...
    def LSTM_op(self):
            ...
            with tf.variable_scope("biLSTM_word", reuse=tf.AUTO_REUSE):
                ...
                self.word_level_output, self.alphas_w = Myattention.attention(outputs_w, self.max_sent_len,
                                                                              self.hidden_units * 2, return_alphas=True,regularizer=self.regularizer,
                                                                              name="word_level")
            ...
            with tf.variable_scope('proj'):
                w = tf.get_variable(name='w',shape =[ self.hidden_units * 2 , self.num_tags],initializer=self.initializer, regularizer=self.regularizer,dtype=tf.float32)
                b = tf.get_variable(name='b',shape=[self.num_tags],initializer=tf.zeros_initializer,dtype=tf.float32)

                s = tf.shape(x2)

                outputs_s = tf.reshape(x2, [-1, 2*self.hidden_units])
